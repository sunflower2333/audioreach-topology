...
dnl AR_MODULE_PCMENC(index, sgidx, container-idx, iid, maxip-ports, max-op-ports, src-port, dst-port,
...
`                AR_TKN_U32_MODULE_ID STR(MODULE_ID_PCM_ENC)'
...
`                AR_TKN_U32_MODULE_DST_INSTANCE_ID STR($10)'
`                AR_TKN_U32_MODULE_FMT_INTERLEAVE STR($9)'
`        }'
`}'
`'
`SectionData."NAME_PREFIX.pcm_encoder$1_data" {'
`        tuples "NAME_PREFIX.pcm_encoder$1_tuples"'
`}'
`'
`SectionWidget."NAME_PREFIX.pcm_encoder$1" {'
`        index STR($1)'
`        type "encoder"'
`        no_pm "true"'
`        subseq "10"'
...
`                "NAME_PREFIX.pcm_encoder$1_data"'
`        ]'
`}') dnl
