...
dnl AR_MODULE_PCMDEC(index, sgidx, container-idx, iid, maxip-ports, max-op-ports, src-port, dst-port,
...
`                AR_TKN_U32_MODULE_ID STR(MODULE_ID_PCM_DEC)'
...
`}'
`'
`SectionData."NAME_PREFIX.pcm_decoder$1_data" {'
`        tuples "NAME_PREFIX.pcm_decoder$1_tuples"'
`}'
`'
...
`                "NAME_PREFIX.pcm_decoder$1_data"'
`        ]'
`}') dnl
